...
//        }
   ...
   private static void decorate(Character bot) {
        ...
        decorateEquipment(bot);

        BotDecorateNX.apply(bot);

        applyStyleDecorator(bot);
    }

    private static void applyStyleDecorator(Character bot) {
        switch (bot.getStyle()) {

            case HENEHOE:
                HenehoeDecorator.apply(bot);
                break;
    ...
    private static void decorateEquipment(Character bot) {

        if (!EquipMetadataCache.isInitialized()) {
            QuickEquip.apply(bot);

            BotDecorationQueue.addBot(
                    "default",
                    bot.getId()
            );
    ...
    private static boolean hasClothing(Character bot) {
        ...
        return bot.getInventory(InventoryType.EQUIPPED)
                .list()
                .stream()
                .anyMatch(item ->
                        item.getPosition() == -5 ||
    ...
    public static void setBotVariables(
        ...
        int level = (minLevel >= maxLevel)
                ? minLevel
                : generateBotLevel(tier, minLevel, maxLevel);

        int job = (forcedJobId > 0)
                ? forcedJobId
                : selectJobForClass(baseClass, level);
